...
class Stage extends MusicBeatState
{
		...
		if (!FlxG.save.data.optimize && FlxG.save.data.background)
		{
			switch (daStage)
			{
				case 'voltexStage':
					...
					camZoom = 0.65;
					var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('stageBacks/' + PlayState.SONG.songId.toLowerCase()));
					bg.antialiasing = FlxG.save.data.antialiasing;
					bg.screenCenter();
					swagBacks['bg'] = bg;
					if (PlayState.SONG.songId.toLowerCase() == 'bi')
						bg.scale.set(1.082648871, 1.097966728);
					toAdd.push(bg);

					/*var rasus = new FlxSprite(-600,
							-200).loadGraphic(Paths.image('FK4TNKmXMAMJsY3')); //  Can we pretend that airplanes in the night sky are like shooting stars?
						rasus.antialiasing = FlxG.save.data.antialiasing;
						swagBacks['rasus'] = rasus;
						rasus.screenCenter(); */

					// Finale sprites for Rasis' song 2nd half.
					if (PlayState.SONG.songId.toLowerCase() == 'i')
					{
						var bg2:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('BG_back_finale'));
						bg2.antialiasing = FlxG.save.data.antialiasing;
						swagBacks['bg2'] = bg2;
						bg2.visible = false;
						bg2.screenCenter();
						var stageFront:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('BG_floor_symmetrical'));
				...
				switch (curStage)
				{
					case 'voltexStage':
						if (PlayState.SONG.songId == 'i')
						{
							...
							{
								if (curStep == Std.int(1304 * PlayState.songMultiplier))
								{
									remove(swagBacks['bg']);
	...
	{
		spinCat = FlxTween.tween(swagBacks['coolCatBG'], {angle: 180}, 8 / PlayState.songMultiplier, {
			ease: FlxEase.linear,
			onComplete: function(tween:FlxTween)
			{
				spinCat = null;
				FlxTween.tween(swagBacks['coolCatBG'], {angle: 360}, 8, {
					ease: FlxEase.linear,
					onComplete: function(tween:FlxTween)
					{
						spinCat = null;
	...
	{
		rasisTween = FlxTween.tween(swagBacks['hotGirlBG'], {y: pos + 30}, 2 / PlayState.songMultiplier, {
			ease: FlxEase.sineInOut,
			onComplete: function(tween:FlxTween)
			{
				rasisTween = null;
				rasisTween = FlxTween.tween(swagBacks['hotGirlBG'], {y: pos - 30}, 2, {
					ease: FlxEase.sineInOut,
					onComplete: function(tween:FlxTween)
					{
						rasisTween = null;
		...
		FlxTween.tween(swagBacks['hotGirlBG'], {y: pos + 30}, 2 / PlayState.songMultiplier, {
			ease: FlxEase.sineInOut,
			onComplete: function(tween:FlxTween)
			{
				FlxTween.tween(swagBacks['hotGirlBG'], {y: pos - 30}, 2, {
					ease: FlxEase.sineInOut
				});
			}
		});
