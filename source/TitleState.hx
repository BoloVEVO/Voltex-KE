...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var bg:FlxSprite;
		bg = new FlxSprite(0, 0).loadGraphic(Paths.image('VoltexStage_back_blur', 'shared'));
		bg.screenCenter();
	...
	override function beatHit()
	{
		...
		switch (curBeat)
		{
			...
			// credTextShit.screenCenter();
			case 5:
				createCoolText(['Kade Engine my beloved', 'by']);
			case 7:
				addMoreText('Average4KadeDev LOLOL');
			// credTextShit.text += '\nNewgrounds';
			case 8:
				deleteCoolText();
				ngSpr.visible = false;
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			FlxTween.tween(logoBl, {y: logoBl.y - 100}, 1.4, {ease: FlxEase.expoInOut});
