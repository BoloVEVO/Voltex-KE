...
class ResultsScreen extends FlxSubState
{
	...
	override function create()
	{
		...
		if (!PlayState.isStoryMode)
		{
			songText = new FlxText(20, -65, FlxG.width,
				'Played on ${PlayState.instance.songFixedName} - ${CoolUtil.difficultyFromInt(PlayState.storyDifficulty, PlayState.SONG.songId == 'fin4le').toUpperCase()}');
			songText.size = 34;
		...
		if (PlayState.isStoryMode)
		{
			...
			text.text = 'Week Cleared on ${CoolUtil.difficultyFromInt(PlayState.storyDifficulty, PlayState.storyWeek == 1).toUpperCase()}!';
		}
	...
	override function update(elapsed:Float)
	{
		...
		if (PlayerSettings.player1.controls.ACCEPT)
		{
			...
			#if !switch
			if (PlayState.SONG.validScore && !FlxG.save.data.botplay && !PlayState.addedBotplay && !PlayStateChangeables.practiceMode)
			{
